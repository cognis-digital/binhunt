"""Native cognis-connect emit for binhunt — forward findings to any platform.

Maps binhunt's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    binhunt ... --format json | binhunt-emit --to stix
    binhunt-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('path', '')
        out['severity'] = 'info'
        out['type'] = 'binary_integrity'
        out['description'] = 'Binary integrity finding'
        out['tags'] = ['binary', 'integrity']
