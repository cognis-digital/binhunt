...
**binhunt** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `binhunt` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
binhunt ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
binhunt ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
binhunt ... --format json | cognis-connect emit --to sigma   # Sigma rules
```
...
works whatever `binhunt` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="binhunt") for rec in records]   # records = your JSON output
