...
How **binhunt** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `binhunt` lives in the **Malware / binary** cluster.
...
| from | relation | to |
|---|---|---|
| `binhunt` | JSON findings compose with | [`yaragen`](https://github.com/cognis-digital/yaragen), [`entropyscan`](https://github.com/cognis-digital/entropyscan) |
| `binhunt` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `binhunt` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `binhunt` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the malware / binary cluster.** `binhunt` output feeds the next tool:
```bash
binhunt ... --format json > out.json          # this tool's findings (see `binhunt --help`)
...
binhunt ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
binhunt ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
binhunt ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `binhunt`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `binhunt` slots
